...
function Valide({ items, email,id }) {
  ...
  if (items.Status == "VALIDE" && items.email == email) {
    return (
      <>
        <Box
        mt={2}
...
function Cancel({ items, email,id }) {
  ...
  if (items.Status == "ANNULE" && items.email == email) {
    return (
     <>
          <Box
        mt={2}
...
function Regle({ items, email,id }) {
  // console.log(items.Status);
  if (items.Status == "REGLE" && items.email == email) {
    return (
     <>
        <Box
        mt={2}
...
export default function HistDev() {
  ...
  return (
    <>
      ...
      <Center  mt={10} width="100%" display={"grid"}>
